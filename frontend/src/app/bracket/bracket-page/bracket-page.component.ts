...
import { TournamentBracketData, BracketMatch, MatchParticipant } from "../../models/bracket.models";
...
@Component({
    ...
    imports: [
        BracketTitleComponent,
        StageTabsComponent,
        GroupStageComponent,
        BracketLayoutComponent,
        BracketConnectorComponent,
        GrandFinalComponent,
        ChampionCardComponent,
        GrandFinalPopupComponent
    ],
...
export class BracketPageComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    // will determine who will win the next battle
    winnerSet = signal(new Set<number>());

    ngOnInit(): void {
        this.tournamentId = Number(this.route.snapshot.paramMap.get("id"));
        this.loadBracket();
        this.sub = this.eventBus
            .on<{ participant: MatchParticipant; opponent: MatchParticipant }>("bracket-winner:select")
            .subscribe(participants => {
                const { participant, opponent } = participants;

                // toggle the participant id within the winner set
                this.winnerSet.update(set => {
                    const newSet = new Set(set);
                    if (newSet.has(participant.playerId)) {
                        newSet.delete(participant.playerId);
                    } else {
                        newSet.add(participant.playerId);
                    }
                    newSet.delete(opponent.playerId);
                    return newSet;
                });

                console.log(this.winnerSet());
            });
    }
    ...
    simulateMatch(): void {
        ...
        this.service.oneMatch(this.tournamentId, this.winnerSet()).subscribe({
            next: d => {
                const { resolvedIds, arrivingKeys } = this.diffMatches(before, d);
                this.data.set(d);
                this.battleEffects.trigger(resolvedIds, arrivingKeys);
    ...
    private snapshotMatches(
        d: TournamentBracketData | null
    ): Map<number, { complete: boolean; p1: boolean; p2: boolean }> {
